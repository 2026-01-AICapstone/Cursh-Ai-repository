...
#         req_last_idx = mask_unsafe_sample_request.shape[-1] - 1
#
#         # ✅ 중심점 업데이트는 루프 밖에서 마지막 타겟 레이어 기준으로 1번만
#         #h_un_for_centroid = lora_unsafe_outputs[target_layers_unsafe[-1]][:, req_last_idx, :]
#         h_un_for_centroid = orig_unsafe_hidden[-1, :, req_last_idx, :]
#         current_centroids = self.centroid_mgr.update_centroids(
#             h_un_for_centroid.detach(), labels
#         )
#
#         for layer_idx, layer_num in enumerate(target_layers_unsafe):
#             h_un_new = lora_unsafe_outputs[layer_num][:, req_last_idx, :]
...
#             # 벡터 변화량
#             with torch.no_grad():
#                 h_un_new_norm = F.normalize(h_un_new.float().detach(), p=2, dim=-1)
#                 h_un_orig_norm = F.normalize(h_un_orig.float().detach(), p=2, dim=-1)
...
#                 h_sa_orig = h_sa_new
#
#             # ✅ current_centroids는 루프 밖에서 이미 계산된 값 재사용
#             l_b, l_pull, l_push = calc_crush_loss(
#                 h_sa_orig, h_sa_new, h_un_orig, h_un_new, labels, current_centroids,
#                 m_b=1.0, m_pull=1.0, m_push=1.0
#             )
